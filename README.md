...
| `chunk_size` | int | `25000` | 1 chunk あたりの目安文字数。Claude Code の tool-result 上限 (~25k tokens) を日本語含めて超えない値。max `100000`。見出し境界優先で分割 |
| `timeout_ms` | int | `30000` | ネットワークtimeout |
| `compress` | bool | `false` | genshijin 圧縮を有効化。散文の冠詞/敬語語尾/クッション/ぼかし/フィラー削除。**コード/URL/識別子/パス/バージョン番号は保護**。技術記事 1-3% 削減、散文記事 5-15% 削減 |
...
chunk: 1/3 (24812 chars of 78421 total)
extracted_main: true
compressed: false
...
<!-- mcp-genshijin-fetch meta
source: ...
chunk: 1/1 (13505 chars of 13505 total)
extracted_main: true
compressed: true (genshijin, 13792 → 13505 chars, 2.1% reduction)
has_next: false (final chunk)
-->
```

### 圧縮との対比 (vs WebFetch Haiku 要約)

| | WebFetch Haiku | mcp-genshijin-fetch (compress: true) |
|---|---|---|
| 削減率 | 50-90% | 1-15% |
| 情報損失 | あり (要約) | なし (散文圧縮のみ、コード/URL/識別子保護) |
| 透明性 | サイレント | meta header に圧縮率明示 |
| 用途 | 速読 | context 節約しつつ細部保持 |
...
- [x] genshijin圧縮統合 (`fetch` に `compress: true` オプション)
